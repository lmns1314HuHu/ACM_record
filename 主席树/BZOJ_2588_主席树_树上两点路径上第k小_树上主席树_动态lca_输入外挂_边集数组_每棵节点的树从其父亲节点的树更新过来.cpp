//本题是让求树上两点路径之间的第k大，树上主席树+动态lca+输入外挂
//bzoj上需要输入外挂，而且要用边集数组来存边，用vector可能会TLE
...
{
    while(~scanf("%d%d", &n, &m)){
        int pre = 0;//题目要求，要求记录前一个值，并且要与下一个询问的前边的那个范围做异或操作
        for (int i = 0; i < n; i++) val[i] = iread();
        ...
        while(m--){
            x = iread(),y = iread(), z = iread();
            x = x^pre;
            int father = query_lca(x, y);
            int fatherhash = lower_bound(valc, valc+len, val[father-1])-valc;
            pre = valc[query_segtree(tr[x], tr[y], tr[father], 0, len-1, z, fatherhash)];
            printf("%d", pre);
            if(m != 0) printf("\n");
        }
    }
